...
if(!dead){
	if (hp <= 0)
	{
		...
		//ranged attacking
		if(attacking && !ranged_attack_cd){
			//audio_play_sound(sfx_magic_ranged_attack)
			instance_create_layer(x,y,layer,obj_skele_mage_attack)
			obj_skele_mage_attack.damage = ranged_attack_dmg
			ranged_attack_cd = true
			attacking = false
			alarm[1] = room_speed * ranged_attack_spd
		}
	}
}